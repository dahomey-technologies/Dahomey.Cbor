...
* **It describes what the serializer writes, not what the reader accepts.** `UnhandledNameMode`
  defaults to `Silent`, so unknown keys are tolerated on read but never emitted — the schema is exact
  and closed, matching the writer, not a looser description of everything the reader tolerates.
* **Member types follow their nullable annotations.** A member whose type is annotated nullable (or
  ...
  bare rule `X`. Integer members narrower than 32 bits (`byte`, `short`, etc.) get their exact range,
  not a generic `int`.
* **Polymorphic types get a rule pair.** A base type `X` gets both `X`, describing its own members, and
  `X-poly`, the type choice `X-poly = A-poly / B-poly / ...` over its discriminated subtypes (or
  `A / B-poly / ...` when a concrete subtype has none of its own). A member typed as the polymorphic
  base references `X-poly`, not `X`. An abstract type or interface with no discriminated subtype
  reachable from the context is a build error (`CBOR1008`) — a type choice with nothing to distinguish
  its arms describes a document nothing can actually tell apart.
...
it against the reference [`cddl`](https://rubygems.org/gems/cddl) Ruby gem.
